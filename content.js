...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    } else if (request.action === 'toggleManual') {
    ...
    } else if (request.action === 'updateAutoSkip') {
        autoSkipEnabled = request.enabled;
        console.log(`AutoSkip updated: ${autoSkipEnabled}`);
...
function solveLoop() {
    ...
    if (manualEnabled) {
        ...
        const noMistakeButton = potentialWords.find(el => {
            const text = el.innerText ? el.innerText.replace(/[\u2018\u2019\u02BC\u0060]/g, "'").replace(/\s+/g, ' ').trim().toUpperCase() : '';
        ...
        const sentenceWords = potentialWords.filter(el => {
            ...
            const upperText = text.replace(/[\u2018\u2019\u02BC\u0060]/g, "'").toUpperCase();
                        ...
                        if (answer) {
                            if (answer.type === 'no_mistake') {
                                ...
                                if (!btn) {
                                    ...
                                    btn = allBtns.find(b => {
                                        const t = b.innerText ? b.innerText.replace(/[\u2018\u2019\u02BC\u0060]/g, "'").toUpperCase() : '';
    ...
    const isNonExerciseScreen = !hasExerciseWords;
    const skipButton = isNonExerciseScreen ? allButtons.find(btn => {
    ...
    // Auto-skip or manual check transitions
...
function removeDiscreteHighlights() {
    const highlighted = document.querySelectorAll('[data-pv-highlighted]');
    highlighted.forEach(el => {
        el.style.borderBottom = '';
        el.style.border = '';
        el.style.outline = '';
        el.removeAttribute('data-pv-highlighted');
    });
}

// --- Global Keyboard Shortcuts (New Features!) ---

window.addEventListener('keydown', (e) => {
    if (e.altKey && e.shiftKey) {
        const key = e.key.toLowerCase();
        
        // 1. Alt + Shift + S : Stop / Désactiver
        if (key === 's') {
            e.preventDefault();
            console.log('Projet Voltaire Assistant: Inactif triggered via Alt+Shift+S.');
            chrome.storage.local.set({ solveEnabled: false, manualEnabled: false }, () => {
                solveEnabled = false;
                manualEnabled = false;
                removeDiscreteHighlights();
                
                chrome.runtime.sendMessage({
                    action: 'stateToggledExternally',
                    solveEnabled: false,
                    manualEnabled: false
        ...
        // 2. Alt + Shift + D : Activer le mode Discret (Manuel)
        else if (key === 'd') {
            e.preventDefault();
            console.log('Projet Voltaire Assistant: Mode Discret triggered via Alt+Shift+D.');
        ...
        // 3. Alt + Shift + A : Activer le mode Automatique
        else if (key === 'a') {
            e.preventDefault();
            console.log('Projet Voltaire Assistant: Mode Automatique triggered via Alt+Shift+A.');
        ...
        // 4. Alt + Shift + F : Résoudre et valider uniquement la phrase en cours
        else if (key === 'f') {
            e.preventDefault();
            console.log('Projet Voltaire Assistant: Force solving current sentence via Alt+Shift+F.');
            forceSolveCurrentSentence();
        }
    }
});

function forceSolveCurrentSentence() {
    const potentialWords = Array.from(document.querySelectorAll(config.wordSelector)).filter(isVisible);
                ...
                if (answer) {
                    if (answer.type === 'no_mistake') {
                        ...
                        if (btn) {
                            simulateClick(btn);
                            incrementStat('correct');
                            setTimeout(clickNextOrValidateButton, 500);
                        }
                    } else if (answer.type === 'click_word') {
                        const normalizedAnswer = normalizeSentence(answer.word);
                        let targetWord = sentenceWords.find(w =>
...
function clickNextOrValidateButton() {
    const allButtons = Array.from(document.querySelectorAll('button, [data-testid="button"]')).filter(isVisible);
    const validerBtn = allButtons.find(btn => {
        const text = (btn.innerText || btn.textContent || '').trim().toUpperCase();
        return text.includes('VALIDER') || text.includes('SUIVANT') || text.includes('CONTINUER');
    });
    if (validerBtn) {
        simulateClick(validerBtn);
    }
